  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      if (messageId.startsWith("temp-")) {
        toast.error("Aguarde o envio da mensagem ser concluído");
        return;
      }
      ...
      <div className="flex items-center justify-between gap-2 border-b border-slate-800 bg-slate-900 px-3 py-3 sm:px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Macros — manually-triggered automations (e.g. "Ativar Agente")
              only rendered when the user has at least one configured. Icon-only
              (no text label, unlike Status/Assign) so it doesn't squeeze the
              contact name/phone out of room in the header's fixed width. */}
          {macros.length > 0 && (
            <DropdownMenu>
              <DropdownMenuTrigger
                title="Macros"
                aria-label="Macros"
                className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-slate-400 hover:bg-slate-800"
              >
